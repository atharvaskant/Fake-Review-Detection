...
result_stfd = data.frame(LabeledData=integer(),SupervisedAcc=double(),SemiSupervisedAcc=double())
...
for (i in 1:length(nldx)){
  ...
  result_stfd[i,]=c(nldx[i],sup.accuracy,unsup.accuracy)
  
}
nbs=ggplot(data=result_stfd)+
  ggtitle("Supervised vs Semi-Supervised using Naïve Bayes (Stratified Sampling)")+
